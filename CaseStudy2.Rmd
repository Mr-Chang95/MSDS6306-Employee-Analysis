...
As we build and analyze our KNN model, we find that KNN is not the best model to use. Our accuracy is abysmal. Our sensitivity is only slightly better at 0.7446 while is worse at 0.6119. As a result, we will move on to a different model.   
```{r}
...
## Naive-Bayes Model
Now, want to use Naive-bayes to build our model. This model yielded us much better results with an accuracy of 0.8884, a sensitivity of 0.9019 and specificity 0.8750. As a result, we find this model to be quite accurate. Therefore, the Naive-bayes model that we've built to predict attriton on the test dataset.   
```{r}
set.seed(25)
